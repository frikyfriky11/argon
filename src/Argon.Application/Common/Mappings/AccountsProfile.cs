...
public class AccountsProfile : Profile
{
  ...
  {
    CreateMap<Account, AccountsGetListResponse>()
      .ForCtorParam(nameof(AccountsGetListResponse.TotalAmount), options => options.MapFrom(account => account.TransactionRows.Sum(x => (x.Debit ?? 0) - (x.Credit ?? 0))));
